 ...
 * @license		http://afaphp.com/license.html
 */
abstract class Captcha
{
	...
	public static function instance($group = 'default')
	{
		if ( ! isset(Captcha::$instance))
		{
			...
			$config = F::config('captcha.'.$group);
			// Set the captcha driver class name
			
			$class = 'Captcha_'.ucfirst($config['style']);
			
			//载入driver 文件
			include_once DRIVERPATH.'captcha/'.$config['style'].EXT;
	...
	public function __construct($group = NULL)
	{
		...
		if ( ! is_array($config))
			throw new Exception("Captcha $group not defined in :Captcha $group configuration in config.php curl.$group", E_WARNING);

		// All captcha config groups inherit default config group
		if ($group !== 'default')
		{
			// Load and validate default config group
		    $default = F::config('captcha.default');
			if ( ! is_array($default))
				throw new Exception("Captcha default not defined in :Captcha default configuration in config.php curl.default", E_WARNING);
		...
		if ( ! empty($config['background']))
		{
			...
			if ( ! is_file(Captcha::$config['background']))
				throw new Exception('The specified file, '.Captcha::$config['background'].', was not found.', E_WARNING);
		}
	...
	public function image_render($html)
	{
		// Output html element
		if ($html === TRUE)
			return '<img src="'.input::uri('base').'captcha/'.Captcha::$config['group'].'" width="'.Captcha::$config['width'].'" height="'.Captcha::$config['height'].'" alt="Captcha" class="captcha" />';
