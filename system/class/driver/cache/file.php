...
class Cache_File extends Cache{
	...
	public static $cache_dir = './application/cache';
	...
	protected function __construct(array $config)
	{
		...
		try
		{
			$directory = isset($this->_config)?$this->_config['cache_dir']:die('not dir cache');
		...
		{
			die('Unable to create cache directory as a file already exists : :resource'.$this->_cache_dir->getRealPath());
		}

		// Check the read status of the directory
		if ( ! $this->_cache_dir->isReadable())
		{
			die('Unable to read from the cache directory :resource'.$this->_cache_dir->getRealPath());
		}

		// Check the write status of the directory
		if ( ! $this->_cache_dir->isWritable())
		{
			die('Unable to write to the cache directory :resource'. $this->_cache_dir->getRealPath());
		}
	}
	...
	public function set($id, $data, $lifetime = NULL)
	{
		...
		if ($lifetime === NULL)
		{
			...
			$lifetime = isset($this->_config)?isset($this->_config['default_expire'])?$this->_config['default_expire']:$this->DEFAULT_EXPIRE:$this->DEFAULT_EXPIRE;
		}
		...
		if ( ! $dir->isDir())
		{
			...
			{
				die(__METHOD__.' unable to create directory : :directory: '. $directory);
			}
		...
		try
		{
			...
			if ($file->isFile())
			{
				...
				catch (ErrorException $e)
				{
					...
					{
						die(__METHOD__.' failed to delete file : :file: '.$file->getRealPath());
					}
				}
			}
			// Else, is directory
			elseif ($file->isDir())
			{
				...
				catch (ErrorException $e)
				{
					...
					if ($e->getCode() === E_WARNING)
					{
						die(__METHOD__.' failed to delete directory : :directory:'.$file->getRealPath());
					}
					throw $e;
				}
			}
