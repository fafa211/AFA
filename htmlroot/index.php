...
if(isset($path_vars[1]) && $path_vars[1])  $cModule = $path_vars[1];//模型名
if(isset($path_vars[2]) && $path_vars[2])  $cController = $path_vars[2];
...
if(isset($modules[$cModule])){
    ...
    if (file_exists($cFile)){
        include($cFile);
        if (isset($path_vars[3])  && $path_vars[3]) $cMethod = $path_vars[3];//参数
        if (isset($path_vars[4])  && $path_vars[4]) $cParams = array_slice($path_vars, 4);//参数
        ...
        if (file_exists($cFile)) {
            include($cFile);
            $cController = $cModule;
            if(isset($path_vars[2])  && $path_vars[2])  $cMethod = $path_vars[2];//方法名
            if (isset($path_vars[3])  && $path_vars[3]) $cParams = array_slice($path_vars, 3);//参数
        }else{
            throw new AfaException("Controller {$cController}:{$cFile} not exist! ", E_ERROR);
        }
    }
    $useModule = true;
}else {
